angular
  ...
  .controller(
    ...
    function (
      ...
      if (
        $location.$$path.startsWith("/experiment/edit/") &&
      ...
      ctrl.remove_reference = function (ref) {
        if (
          ref.hasOwnProperty("action") &&
          (ref["action"] == "new" || ref["action"] == "loaded")
        ) {
      ...
      ctrl.isValue = function (value) {
        return typeof value != "object" || value.hasOwnProperty("filename");
      };
    ...
    */
      ...
      ctrl.expname_check_duplicate = function () {
        $scope.duplicateExpName =
          $scope.exp.name && ctrl.expNameList.indexOf($scope.exp.name) > 0;
      };
      ...
      ctrl.ref_check_duplicate = function (ref) {
        ...
        if (
          ref.pubmed_id &&
          ctrl.refPubMedIdMap.hasOwnProperty(ref.pubmed_id)
        ) {
          //PubMedId already is in db
          if (
            (typeof ref.id == "string" && ref.id.startsWith("new_")) ||
      ...
      ctrl.uploadAttachment = function (fieldId, valueId) {
        ...
        att.$save().then(function () {
          uuid = att.uuid;
          $scope.exp.fields[fieldId][valueId] = {
            filename: filename,
            uuid: uuid,
          };
        });
      ...
      ctrl.delete = function () {
        ...
        $scope.warningMessage = $sce.trustAsHtml(
          "Deleting an experiment cannot be undone. " +
