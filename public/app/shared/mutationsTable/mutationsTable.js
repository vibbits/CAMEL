      ...
      link: function (scope, elem, attr) {
        ...
        scope.fieldComparator = function (exp1, exp2) {
          ...
          if (scope.orderParams.realm == "exp") {
            return exp1.value[scope.orderParams.key].toLowerCase() <
              exp2.value[scope.orderParams.key].toLowerCase()
              ? -1
              : 1;
          }
          ...
          if (scope.orderParams.realm == "fields") {
            ...
            if (exp1.value.fields.hasOwnProperty(scope.orderParams.key)) {
              allValues1 = Object.values(
                exp1.value.fields[scope.orderParams.key]
              );
              field1 = allValues1[0];
              if (
                scope.orderParams.type == "value_VARCHAR" ||
                scope.orderParams.type == "value_TEXT"
              ) {
            ...
            if (exp2.value.fields.hasOwnProperty(scope.orderParams.key)) {
              allValues2 = Object.values(
                exp2.value.fields[scope.orderParams.key]
              );
              field2 = allValues2[0];
              if (
                scope.orderParams.type == "value_VARCHAR" ||
