angular
  ...
  .config([
    ...
    function ($routeProvider) {
      $routeProvider
        ...
        .when("/mutation/:id", {
          templateUrl: "app/components/mutation/mutationTemplate.html",
          controller: "MutationController",
          controllerAs: "mutation",
        })
        .when("/mutations", {
  ...
  .run(function ($rootScope, $location, $window, AUTH_EVENTS, AuthService) {
    ...
    $rootScope.$on("$routeChangeStart", function (event, next) {
      if (
        next.hasOwnProperty("$$route") &&
        next.$$route.hasOwnProperty("data") &&
        next.$$route.data.hasOwnProperty("protected")
      ) {
